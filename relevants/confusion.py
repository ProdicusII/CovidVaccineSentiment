...
confuses=      [  [dict(),dict()] , [dict(),dict()]  ]

encs=dataset.encodings['input_ids']
encs=np.array(encs)

for vesnaclass in range(2):
    for algclass in range(2):
...
filename = './confuses_relevants.pickle'
...
conf['nik']=8
conf.to_excel('confusion_relevants.xlsx')